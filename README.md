...
**[opsgate](https://github.com/Andrey834/opsgate)** — AI-агент для эксплуатации Kubernetes через MCP-инструменты (Spring AI 2.0, Streamable HTTP): читает состояние кластера свободно, но каждый деструктивный вызов (`scale_deployment`, `restart_deployment`) перехватывается approval-гейтом до исполнения — модель может только предложить, реально выполнить может только человек через отдельное `POST /approvals/{id}/approve`. Деструктивные вызовы дополнительно защищены подписанным HMAC-токеном, который mcp-server проверяет независимо от agent — не полагаясь на то, что в его коде не окажется бага. `ChatMemory` по conversation id, append-only audit-лог, весь путь подтверждён живым end-to-end прогоном на kind-кластере с реальным вызовом LLM.
